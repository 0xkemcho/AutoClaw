...
export class IchiVaultAdapter implements VaultAdapter {
	...
	async deposit(params: DepositParams): Promise<TxResult> {
		const { vaultAddress, amount, depositor, walletClient, publicClient } =
			params;
		...
		for (let attempt = 0; attempt < HYSTERESIS_MAX_RETRIES; attempt++) {
			try {
				...
				if (msg.includes("try later") || msg.includes("hysteresis")) {
					console.warn(
						`[ichi] Deposit attempt ${attempt + 1}/${HYSTERESIS_MAX_RETRIES} failed (hysteresis). Retrying in ${HYSTERESIS_RETRY_DELAY_MS / 1000}s...`,
					);
	...
	async withdraw(params: WithdrawParams): Promise<TxResult> {
		const { vaultAddress, shares, recipient, walletClient, publicClient } =
			params;
	...
	async getPosition(
		...
		const [shares, totalAmounts, totalSupply, token0, token1] =
			await Promise.all([
				client.readContract({
	...
	private async getDepositTokenAddress(
		vaultAddress: Address,
		client: PublicClient,
	): Promise<Address> {
